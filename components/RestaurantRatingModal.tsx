...
interface RestaurantRatingModalProps {
  ...
  onClose: () => void;
  onSubmitReview: (orderId: string, restaurantId: string, rating: number, comment: string) => Promise<void>;
}
...
const RestaurantRatingModal: React.FC<RestaurantRatingModalProps> = ({ order, restaurant, onClose, onSubmitReview }) => {
  ...
  const handleSubmit = async () => {
    ...
    setIsLoading(true);
    await onSubmitReview(order.id, restaurant.id, rating, comment);
    setIsLoading(false);
    handleClose();
  };
