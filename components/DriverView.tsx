...
interface DriverViewProps {
  driver: Driver; // Changed from drivers: Driver[]
  orders: Order[];
...
const DriverView: React.FC<DriverViewProps> = ({ driver, orders, restaurants, customers, updateOrderStatus, acceptOrder, onUpdateDriver }) => {
  ...
  useEffect(() => {
    // Onboarding logic now runs when the component mounts with the driver prop
  ...
  const handleAcceptOrder = async (orderId: string) => {
    // No need to check for selectedDriver anymore
  ...
  const activeOrder = orders.find(o => o.driverId === driver.id && o.status !== OrderStatus.DELIVERED);
      ...
      <div className="flex justify-between items-center mb-8 flex-wrap gap-4">
        ...
        <div className="flex items-center space-x-2">
            ...
            >
             ...
             {/* Removed the "Switch Driver" button */}
        </div>
      </div>
